...
mod forge_tests {
    ...
    fn push__branches_go_before_main() {
        // Branches, then main, then tags last: a failed lease on main must
        // never leave tags already force-rewritten (EPIC-09 Decision 9, as
        // corrected). This is the order once main already exists on the
        // remote — see `push_commands__a_fresh_remote_gets_main_first` for a
        // first publish.
    ...
    #[test]
    fn push_commands__a_straight_line_pushes_main_then_tags() {
        // Main already exists on the remote, so the ordinary order holds:
        // main goes before tags, exactly the pre-branch order — tags always
        // go last.
