...
| 5 | **Does Forgejo mark a PR merged when main is pushed past it?** Carried from EPIC-09 open question 1, settled there for GitHub only (`docs/spikes/pr-remote/`): a plain fast-forward push of main to Bower's merge commit turns the open PR merged, and stepping stones (EPIC-09 Decision 20) depend on it. Forgejo may need its "manually merged" repository setting. Settle it against a Forgejo container before `ForgejoForge` ships; if it cannot be made to work, a merged branch's PR on Forgejo is recorded in `PULLS.md` and not opened. |
