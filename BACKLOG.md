...
| **Parts** | [`docs/EPIC-17_Chapter_Grouping.md`](docs/EPIC-17_Chapter_Grouping.md) | ✅ Built 19 September 2026 on `epic-17-parts`, awaiting its PR. `# Part` titles in `SUMMARY.md` now reach the epub's table of contents and the PDF as a page each, beside the HTML mdBook already drew. A `PartMark` sits beside the flat chapter list; an empty part is a load error; `site_fingerprint` covers marks. The sample book has three parts. No plan, lock, tag, or SHA moved. Open questions 2, 3 and 5 stay open, none blocking. |
