...
| **Parts** | [`docs/EPIC-17_Chapter_Grouping.md`](docs/EPIC-17_Chapter_Grouping.md) | Group chapters under mdBook's `# Part` titles in `SUMMARY.md`. Filed 19 September 2026. Spiked: the HTML already shows parts, because mdBook draws them; the epub and the PDF drop them silently. Adds a `PartMark` beside the flat chapter list, one divider file per part for pandoc, and part marks in `site_fingerprint`, which a renamed part does not change today. No plan, lock, tag, or SHA moves. Six phases; three open questions, none blocking (1, a flat divider; 4, the sample's parts; and 6, an empty part is a hard error, were answered the same day). |
